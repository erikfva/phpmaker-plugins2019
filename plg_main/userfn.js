...
function resizeIFRM(data){
	...
	var wwidth = $('main-header.navbar').length? $('main-header.navbar').width():  $('html').width() || 0;

	iframe.style.width = data.width > 0 && data.width > wwidth? (data.width + 'px'): '100%';
	...
	document.body.style.width = (data.width < wwidth? wwidth : data.width + 20) + 'px';
...
document.addEventListener('DOMContentLoaded', function() { //window.onload = function(){
	...
	(!isCrossOrigin()) && top.$("html, body").animate({scrollTop: 0, scrollLeft: 0 }, 100);
